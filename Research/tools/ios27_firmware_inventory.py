...
def digest(path:Path)->str:
    ...
    return h.hexdigest()
ap=argparse.ArgumentParser(); ap.add_argument("--root",type=Path,required=True); ap.add_argument("--kernel",type=Path,required=True); ap.add_argument("--device",required=True); ap.add_argument("--build",required=True); ap.add_argument("--out",type=Path,required=True); ns=ap.parse_args()
